...
import mapData from "@/data/map.json"; // Assuming you have a JSON file with map data
            ...
            <div className="container mx-auto px-4">
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="relative border border-gray-800 rounded-md overflow-hidden bg-[#0d0d0d]">
                        {/* Map container */}
                        <div className="relative w-full h-64 md:h-80" onMouseMove={handleMouseMove}>
                            {/* Grid lines */}
                            ...
                            {realms.map((realm) => (
                                <div
                                    ...
                                    className={`absolute w-3 h-3 rounded-full cursor-pointer transition-all duration-300 ${
                                        activeRealm === realm.id ? "bg-gray-300 shadow-[0_0_10px_rgba(255,255,255,0.5)]" : "bg-gray-600 hover:bg-gray-500"
                                    }`}
                                    style={{
                                        left: `${realm.x}%`,
                                        top: `${realm.y}%`,
                                ...
                                >
                                    ...
                                    {/* Realm name tooltip */}
                                    <div
                                        className={`absolute bottom-full left-1/2 transform -translate-x-1/2 -translate-y-2 whitespace-nowrap bg-gray-900 px-2 py-1 rounded text-xs text-gray-300 transition-opacity ${
                        ...
                        {activeRealm && (
                            <Card className="m-4 bg-gray-900/60 border-gray-800">
                                <CardContent className="p-4">
                                    ...
                                    <p className="text-xs text-gray-400">{realms.find((r) => r.id === activeRealm)?.description}</p>
                                </CardContent>
                            </Card>
                        )}
                    </div>
                    ...
                    <div className="mt-4 text-center">
                        <p className="text-xs text-gray-600 font-jetbrains">
                            <span lang="ja" className="text-gray-500">
                                地図の伝説
                            </span>{" "}
                            • Map Legend • <span lang="ja">八領域</span>
                        </p>
                    </div>
                </div>
            </div>
