        ...
        <header className="sticky top-0 z-50 py-4 px-4 bg-[#0a0a0a]/90 backdrop-blur-md border-b border-[#333]">
            <div className="container mx-auto flex justify-between items-center">
                <div className="flex items-center">
                    <span className="font-jetbrains text-lg font-bold text-gray-300 tracking-tight">&lt;world_operator&gt;</span>
                </div>

                {/* Desktop Navigation */}
                <nav className="hidden md:flex items-center space-x-8">
                    {menuItems.map((item) => (
                        <a key={item.name} href={item.href} className="flex items-center font-jetbrains text-xs text-gray-400 hover:text-gray-200 glitch-hover" data-text={item.name}>
                            {item.icon}
                            {item.name}
                        </a>
                    ))}
                ...
                {/* Mobile Menu Button */}
                <button className="md:hidden p-2 rounded-md text-gray-400 hover:text-gray-200" onClick={() => setIsOpen(!isOpen)}>
